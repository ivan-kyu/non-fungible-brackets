...
async function signClaim(
  ...
  assert(process.env.DIAMOND_CONTRACT !== undefined, "DIAMOND_CONTRACT is not set");
  ...
  const dataClaim = nfbDiamond.interface.encodeFunctionData(
    "claim",
    [poolId, tokenId]
  );

  const resultClaimTx = await signMetaTxRequest(
  ...
  try {
    const response = await fetch(AUTO_TASKER_WEBHOOK_URL, {
      method: "POST",
      body: JSON.stringify(resultClaimTx),
      headers: {
